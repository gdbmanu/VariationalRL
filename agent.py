...
class Agent:
    ...
    def __init__(self, env, ALPHA=0.1, GAMMA=0.9, BETA=0.5):
    ...
    def step(self, Q_soft = None):
        curr_obs = self.env.get_observation()
        action = self.softmax_choice(curr_obs, Q_soft = Q_soft)
        new_obs, reward, done = self.env.act(action)
        return curr_obs, action, new_obs, reward, done
        #self.total_reward += reward

    def softmax(self, obs, Q_soft = None):
        if Q_soft is None:
            Q_soft = self.Q_var
        act_score = np.zeros(self.env.N_act)
        for a in range(self.env.N_act):
            act_score[a] = np.exp(self.BETA * Q_soft[obs, a])
        return act_score / np.sum(act_score)

    def softmax_choice(self, obs, Q_soft = None):
        act_probs = self.softmax(obs, Q_soft = Q_soft)
        action = np.random.choice(len(act_probs), p=act_probs)
        return action

    def softmax_expectation(self, obs, Q = None, Q_soft = None):
        act_probs = self.softmax(obs, Q_soft = Q_soft)
        if Q is None:
...
class Trainer():
    ...
    def KL_diff_err(self, past_obs, past_action, obs, done):
        if done:
            return self.KL_diff(past_obs, past_action, obs) - self.agent.KL_diff[past_obs, past_action]
        else:
    ...
    def run_episode(self):
        ...
        while True:
            past_obs, past_action, obs, reward, done = self.agent.step(Q_soft=self.Q_soft)
            self.nb_visits[obs] += 1
            #reward *= 0
            self.total_reward += reward
            self.agent.KL_diff[past_obs, past_action] += self.agent.ALPHA * self.KL_diff_err(past_obs, past_action, obs, done)
            self.agent.Q_ref[past_obs, past_action] += self.agent.ALPHA * self.TD_err_ref(past_obs, past_action, obs, reward, done)
            self.agent.Q_var[past_obs, past_action] += self.agent.ALPHA * self.TD_err_var(past_obs, past_action, obs, reward, done)
