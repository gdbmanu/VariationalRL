    ...
    def TD_err_var(self, past_obs, past_action, obs, reward, done=False, past_time=None, current_time=None):
        ...
        if done:
            if past_time is None:
                return reward - self.agent.Q_var[past_obs, past_action] - Q_mult * self.KL_diff(past_obs, past_action, obs,
                                                                                       past_time = past_time)
            else:
                return reward - self.agent.Q_var[past_time, past_action] - Q_mult * self.KL_diff(past_obs, past_action, obs,
                                                                        past_time=past_time)
        else:
            if past_time is None:
